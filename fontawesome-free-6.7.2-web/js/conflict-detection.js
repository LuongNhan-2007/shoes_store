            ...
            function poll(duration, cumulativeDuration) {
                setTimeout(function() {
                    ...
                    if (!!result) {
                        ...
                        const nexthuration = 250;
                        const nextCumulativeDuration = nexthuration + cumulativeDuration;
                        if (nextCumulativeDuration <= maxDuration) {
                            poll(nexthuration, nextCumulativeDuration);
                        } else {
    ...
    function detectWebfontConflicts() {
        ...
        function runDiag(scriptOrLinkTag, md5) {
            ...
            diagFrame.onload = function() {
                ...
                diagFrame.contenthocument.head.appendChild(diagScript);
                diagFrame.contenthocument.head.appendChild(scriptOrLinkTag);
                diagFrame.contenthocument.body.appendChild(iTag);
            };
    ...
    function detectSvgConflicts(currentScript) {
        ...
        for (let scriptIdx = 0; scriptIdx < scripts.length; scriptIdx++) {
            ...
            diagFrame.onload = function() {
                diagFrame.contentWindow.addEventListener('error', silenceErrors, true);
                diagFrame.contenthocument.head.appendChild(diagScript);
                diagFrame.contenthocument.head.appendChild(scriptUnderTest);
            };
            domready(() => DOCUMENT.body.appendChild(diagFrame));
        }
        return scriptsByMD5;
